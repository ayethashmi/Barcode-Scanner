...
abstract class DevAppsAbstractBarcodeImageEditorFragment : BaseFragment() {
    fun onBarcodeDetailsActivity(task: (activity: DevAppsBarcodeDetailsActivityDevApps) -> Unit) {
        val activity = requireActivity()
        if(activity is DevAppsBarcodeDetailsActivityDevApps) {
            task(activity)
        } else {
