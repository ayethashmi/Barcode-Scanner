...
class DevAppsProductActionsFragmentDevAppsDevApps: DevAppsAbstractActionsFragmentDevApps() {
    override fun configureActionItems(barcode: Barcode) {
        if(barcode.getBarcodeType() == BarcodeType.UNKNOWN_PRODUCT) {
            addActionItem(configureProductActionItem(barcode))
        } else {
    ...
    private fun configureProductActionItem(barcode: Barcode): ActionItem {
        return ActionItem(
